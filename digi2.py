        ...
        messages=[
            {
                "role": "user",
                "content": [
                    {"type": "text", "text": "Extract ONLY written text from the image."},
...
def main():
    ...
    while True:
        ...
        feedback = gpt4o_chat(f"User Question: {user_input}\n\nAnswer the with ONLY 'YES I WILL SAVE THE NOTES FROM THE SCREEN':")
        ...
        screenshot_text = analyze_image(screenshot_path)
        print(screenshot_text)

        if screenshot_text:
            notes = gpt4o_chat(f"Text from Screenshot = {screenshot_text}\n\nWrite relevant notes from the text above in a structured format:")
            
            # Save the notes to a file named "notes.txt"
            with open("notes.txt", "a") as file:
                file.write(notes)
            
            feedback = gpt4o_chat(f"Notes = {notes}\n\nUser Question: {user_input}\n\nAnswer the with ONLY 'NOTES ARE NOW SAVED':")
            text_to_speech_and_play(feedback)
        else:
            print(f"{YELLOW}No text found in the screenshot.{RESET_COLOR}")
